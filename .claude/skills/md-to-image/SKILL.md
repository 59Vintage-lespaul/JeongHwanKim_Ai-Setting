---
name: md-to-image
description: "마크다운 파일을 브라우저 없이 PNG/SVG 이미지로 변환하는 스킬. marknative 엔진을 사용하여 서버사이드에서 마크다운을 직접 페이지네이션된 이미지로 렌더링한다. 트리거: '이미지로 만들어줘', 'PNG으로 변환', '카드 이미지', '마크다운 이미지', 'md to png', 'md to image', '공유용 이미지', '카드로 만들어줘', '마크다운을 이미지로', '마크다운 렌더링', '페이지 이미지', 사용자가 .md 파일을 업로드하면서 '이미지', 'PNG', 'SVG', '카드' 등을 요청하는 경우, 또는 다른 스킬(tweet-trend-analyzer 등)이 생성한 .md 파일을 이미지로 변환해야 하는 경우. 이 스킬은 마크다운을 이미지로 변환하는 모든 요청에서 적극적으로 트리거해야 한다."
---
...
```
[trend-research가 trend_summary_2026-04.md 생성]
 ↓
bun run scripts/render.ts trend_summary_2026-04.md /mnt/user-data/outputs/ --preset wide
 ↓
...
예: `trend_summary_2026-04.md` → `trend_summary_2026-04_01.png`, `..._02.png`, ...
